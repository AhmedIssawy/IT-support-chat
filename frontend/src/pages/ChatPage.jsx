...
function ChatPage() {
  ...
  useEffect(() => {
    if (!isAdmin || !socket || !authUser) return;

    const handleBeforeUnload = (e) => {
      // Emit terminateChat event to delete all admin's chats
      socket.emit("terminateChat", { adminId: authUser._id });
      
      // Note: Modern browsers ignore custom messages, but we can still trigger the event
      // The socket emit should work even if the page is closing
    };
    ...
    return () => {
      ...
      // Emit terminateChat on unmount as well
